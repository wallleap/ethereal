...
下面两个是用于统计文章的热度和页面访问量的

**`VITE_GIST_TOKEN `**（Gist Token）

前往 [https://github.com/settings/tokens](https://github.com/settings/tokens) 生成一个新的 Token（classic），输入密码

勾选 `Gist` 权限

复制生成的 Token，填入 `.env.local` 中的 `VITE_GIST_TOKEN` 中，并在中间任意位置加入 ` ,` 空格+半角逗号（避免代码部署到 GitHub 后检测到让 Token 过期）

例如：`VITE_GIST_TOKEN="ghp_11AXVFLRY0i183N0naE8i4_C01urvOKNg2bjJWTZ65yL4sAkcsW, 5LClTG25dh2xNNRNKQMSJGZVWswJ9Fq"`
...
`Name` 为 `VITE_TWIKOO_ID`、`VITE_GIST_TOKEN`、`VITE_GIST_ID`、`VITE_CLARITY_ID`
